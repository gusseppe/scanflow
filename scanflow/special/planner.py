    ...
    def choose_port(self, port: int):
        ...
        for i in range(n_trials+1):
            ...
            if self.check_port_in_use(chosen_port):
                logging.info(f"[Planner] Port {chosen_port} is in use. Trying next port.")
                continue
            else:
                logging.info(f"[Planner] Port {chosen_port} is set successfully.")
                return chosen_port

        raise ValueError(f'[Planner] {n_trials} additional ports are in use. Please select another one.')
