...
def trt_engine_op(in_tensor, serialized_segment, OutT, workspace_size_bytes, precision_mode, segment_func="", input_shapes=[], max_cached_engines_count=1, max_batch_size=1, calibration_data="", use_calibration=True, segment_funcdef_name="", cached_engine_batches=[], fixed_input_size=True, output_shapes=[], static_engine=True, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        ...
        "fixed_input_size", fixed_input_size, "output_shapes", output_shapes,
    ...
    try:
      return trt_engine_op_eager_fallback(
          ...
          fixed_input_size=fixed_input_size, output_shapes=output_shapes,
          static_engine=static_engine, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      result = _dispatch.dispatch(
  ...
  if _execute.must_record_gradient():
    _attrs = ("serialized_segment", _op.get_attr("serialized_segment"),
              ...
              "input_shapes", _op.get_attr("input_shapes"),
...
def trt_engine_op_eager_fallback(in_tensor, serialized_segment, OutT, workspace_size_bytes, precision_mode, segment_func, input_shapes, max_cached_engines_count, max_batch_size, calibration_data, use_calibration, segment_funcdef_name, cached_engine_batches, fixed_input_size, output_shapes, static_engine, name, ctx):
  serialized_segment = _execute.make_str(serialized_segment, "serialized_segment")
  if not isinstance(OutT, (list, tuple)):
    raise TypeError(
        "Expected list for 'OutT' argument to "
  ...
  "max_cached_engines_count", max_cached_engines_count, "max_batch_size",
  max_batch_size, "workspace_size_bytes", workspace_size_bytes,
  "precision_mode", precision_mode, "calibration_data", calibration_data,
  "use_calibration", use_calibration, "segment_funcdef_name",
  segment_funcdef_name, "cached_engine_batches", cached_engine_batches,
  "fixed_input_size", fixed_input_size, "output_shapes", output_shapes,
