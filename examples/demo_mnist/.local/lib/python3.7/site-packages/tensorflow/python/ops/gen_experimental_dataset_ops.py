...
def snapshot_dataset_v2(input_dataset, path, reader_func_other_args, shard_func_other_args, output_types, output_shapes, reader_func, shard_func, compression="", reader_prefix="", writer_prefix="", name=None):
  r"""Creates a dataset that will write to / read from a snapshot.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        ...
        writer_prefix, "reader_func", reader_func, "shard_func", shard_func)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
    ...
    try:
      return snapshot_dataset_v2_eager_fallback(
          ...
          writer_prefix=writer_prefix, reader_func=reader_func,
  ...
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "SnapshotDatasetV2", input_dataset=input_dataset, path=path,
                             ...
                             writer_prefix=writer_prefix, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("output_types", _op.get_attr("output_types"), "output_shapes",
              _op.get_attr("output_shapes"), "compression",
              ...
              _op.get_attr("writer_prefix"), "reader_func",
...
def snapshot_dataset_v2_eager_fallback(input_dataset, path, reader_func_other_args, shard_func_other_args, output_types, output_shapes, reader_func, shard_func, compression, reader_prefix, writer_prefix, name, ctx):
  if not isinstance(output_types, (list, tuple)):
    raise TypeError(
        "Expected list for 'output_types' argument to "
        "'snapshot_dataset_v2' Op, not %r." % output_types)
  ...
  writer_prefix, "reader_func", reader_func, "shard_func", shard_func,
  "Treader_func_args", _attr_Treader_func_args, "Tshard_func_args",
  _attr_Tshard_func_args)
