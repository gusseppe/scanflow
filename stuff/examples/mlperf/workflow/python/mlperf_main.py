...
def main():
    ...
    with mlflow.start_run(run_name=f"{args.backend}-{args.model_name}") as mlrun:
        mlflow.log_param(key='backend', value=args.backend)
        mlflow.log_param(key='dataset_path', value=args.dataset_path) 
        mlflow.log_param(key='model', value=args.model) 
        
        
        mlflow.log_metric("accuracy", final_results[str(scenario)]['accuracy'])
        mlflow.log_metric("query_per_second", final_results[str(scenario)]['qps'])
